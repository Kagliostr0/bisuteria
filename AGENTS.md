...
### Cambios seguridad 2026-06-15 (~01:26)
- Backup: `bisuteria-backup-20260615-seguridad`
- **Contexto**: Pentest de OSAI reveló:
  - DEBUG=True en producción (expone páginas de error con variables de entorno)
  - DATABASE_URL con contraseña visible en error pages
  - ALLOWED_HOSTS=* (host header injection)
  - Path traversal funcional (/media/..%2F..%2Fetc%2Fpasswd)
- **Archivos a modificar**:
  1. `requirements.txt` — agregar `django-axes>=6.0`
  2. `bisuteria/settings.py`:
     - `DEBUG` default: `'True'` → `'False'`
     - `ALLOWED_HOSTS` default: `'*'` → `'localhost,127.0.0.1'`
     - Agregar `axes` a INSTALLED_APPS
     - Agregar AxesMiddleware a MIDDLEWARE
     - Configurar AXES_* (failure limit, cooloff, lockout)
  3. `bisuteria/urls.py`:
     - Hardening path traversal (normpath + verificación de directorio)
     - Eliminar password_reset de accounts/ (mantener solo login/logout)
  4. `start.sh`:
     - Usar variable de entorno para password del admin
- **Lo que NO se toca**:
  - Templates (estética intacta)
  - Modelos y migraciones
  - Vistas de la tienda
  - Admin custom (bisuteria/admin.py)
  - Cookie settings (SameSite=Strict ya está bien)
- **Para restaurar**: copiar archivos del backup `bisuteria-backup-20260615-seguridad` sobre el proyecto
- **Pendiente manual en Railway**:
  - Rotar contraseña de PostgreSQL (la actual `loQxOMxDTPBnpvVZFfKVcbNLEjqNCIiJ` está comprometida)
  - Verificar que `DEBUG=False` esté seteado en variables de entorno de Railway
- **Estado**: Cambios ejecutados en local, servidor funcionando
